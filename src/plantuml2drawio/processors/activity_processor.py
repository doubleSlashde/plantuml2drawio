"""Activity diagram processor for converting PlantUML to Draw.io format."""
...
class Node:
    ...
    A node can be an activity, decision, start point, or end point
    in the diagram.
    ...
    def __init__(self, id: str, label: str, type: str):
        ...
        Args:
            id: Unique identifier for the node.
            label: Text displayed in the node.
            type: Type of the node (e.g., "start", "stop", "decision").
        """
        self.id = id
        self.label = label
        self.type = type
...
def layout_activity_diagram(
    ...
    """Layout activity diagram using recursive depth-first search (DFS).

    This function assigns positions to nodes in the activity diagram using
    a column-based approach with DFS traversal.
    """
    # Constants for labels and shapes
    LABEL_START = "start"
    LABEL_STOP = "stop"
    LABEL_MERGE = "merge"
    ...
    def calculate_next_position(
        ...
        if node_by_id[parent_id].type == SHAPE_HEXAGON:
            if index == 0:
    ...
    for node in nodes:
        if isinstance(node, Node):
            node.width = calculate_width(node)
...
def create_activity_drawio_xml(nodes, edges):
    """Create Draw.io XML document from given nodes and edges."""
    # XML header and basic structure
    xml = (
    ...
    for edge in edges:
        ...
        xml += (
            f'        <mxCell id="{edge.id}" value="{edge.label}" '
...
def get_node_style(node):
    ...
    STYLES = {
        ...
        "ellipse": "ellipse;whiteSpace=wrap;html=1;aspect=fixed;",
        "hexagon": (
            "shape=hexagon;perimeter=hexagonPerimeter2;whiteSpace=wrap;"
            "html=1;fixedSize=1;align=center;verticalAlign=middle;"
            "fontFamily=Helvetica;fontSize=12;fontColor=#ffffff;"
            "fillColor=#00A5E1;strokeColor=none;size=10;"
        ),
        "merge": (
            "rhombus;whiteSpace=wrap;html=1;strokeColor=none;"
            "fillColor=#00A5E1;fontColor=#ffffff;"
        ),
        "rectangle": (
            "rounded=1;whiteSpace=wrap;html=1;strokeColor=none;" "fillColor=#DCE9F1;"
        ),
    }
